...
\begin{Representacion}
  ...
  \begin{Estructura}{base}[estr]
      ...
      \dondees{estr}{tupla}$\Big($\emph{tablaMasAccedida}: \TipoVariable{puntero(string)},
        ...
        \emph{tablas}: \TipoVariable{lista(string)}, 
...
\noindent\tab 15) Los registros en la lista de cambios tienen por campos a los campos de la primer clave y, para cada última aparación de un registro en la lista, el bool agregar refleja si pertenece el registro o no a los registros de las primer clave. \\
  ...
  \noindent\tab\tab\bigpar {\textbf{1)} $\big(\forall$ $s$ : string$\big)$ $\Big(s$ $\in$ claves($e$.nombreATabla) $\yluego$ nombre$\big($obtener($s$, $e$.nombreATabla)$\big)$ $\igobs$ $s\Big)$ $\ssi$\\
    ...
    claves$\big($obtener($n$, $e$.hayJoin)$\big)$ $\igobs$ claves$\big($obtener($n$, $e$.nombreATabla)$\big)$} $\land$\\
    ...
    \noindent\tab\tab
      \bigpar{
        ...
        \bigpar{
          ...
          \bigpar{
            $\Big(\forall$ $n$ : nat, def?$\big(n$, obtener$\big(s_2$, obtener($s_1$, $e$.joinPorCampoString)$\big)\big)\Big)$\\
            \bigpar{
