...
\begin{Interfaz}
  ...
  \InterfazFuncion{Borrar}{\In{cr}{registro}, \In{t}{tabla}, \Inout{b}{base}}{}
  [\#campos($cr$) = 1 $\land$ $t$ $\in$ tablas($b$) $\land$ $b$ = $b_0$]
...
\tab Campo no indexado $\implies$  $O\big(|L| * (n + \#tablas(b))\big)$, donde $L$ es el dato string más largo de $cr$ y $n$ es la cantidad de registros en la tabla.]
  ...
  \InterfazFuncion{GenerarVistaJoin}{\In{t_1}{string}, \In{t_2}{string}, \In{c}{campo}, \Inout{b}{base}}{}
  ...
  {$b$ $\igobs$ generarVistaJoin($t_1$, $t_2$, $c$, $b_0$)}
  [\\
...
\tab Donde $n$ = \#registros($t_1$), $m$ = \#registros($t_2$) y $L$ el dato string más largo de cualquiera de las dos tablas.]
  ...
  [hayJoin?($t_1$, $t_2$, $b$)]
  {$res$ $\igobs$ vistaJoin($t_1$, $t_2$, $b$)}
  [ver en los algoritmos para los distintos casos.]
...
\begin{algorithm}[H]{\textbf{iGenerarVistaJoin}(\In{t_1}{string}, \In{t_2}{string}, \In{c}{campo}, \Inout{e}{estr}) $\to$ $res$ : itConj(registro)}
  ...
  \begin{algorithmic}
    ...
    \If{$TipoCampo(c, t1)$} \Comment $O(1)$
      \comentraio{Defino en cada diccionario de joinPorCampoNat la otra tabla.}
