...
\begin{Interfaz}
  ...
  \InterfazFuncion{InsertarEntrada}{\In{r}{registro}, \Inout{s}{string}, \Inout{b}{base}}{}
...
\tab Campo no indexado $\implies$  $O\big(|L| * (n + \#tablas(b))\big)$, donde $L$ es el dato string más largo de $cr$ y $n$ es la cantidad de registros en la tabla.]
  ...
  [\\
  \tab Si $c$ está indexado en alguna de las tablas y es tipo string $\implies$ $O\big(n * |L|)\big)$\\
  \tab Si $c$ está indexado y es nat $\implies$ $O\big(n * (|L|\ +\ log\ m)\big)$\\
...
\begin{algorithmic}
   ...
   \quad\quad Si $c$ está indexado y es nat $\implies$ $O\big(n * (log\ m\ +\ |L|)\big)$ = $O\big((n*m)(log(m)\ +\ |L|)\big)$ \\
...
\begin{algorithm}
  \begin{algorithmic}
   ...
   \quad\quad Su campoIndice es clave, entonces \#coincis = 1, de no serlo está acotada por $n$ (la cantidad de registros de la tabla). \\
