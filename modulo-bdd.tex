...
\begin{Interfaz}
  ...
  \textbf{servicios exportados}: Todos los de la interfaz a excepción de  \tadNombre{CombinarRegistros, Merge y CoincidenTodosCrit}

  \textbf{servicios usados}\hangindent=2\parindent : Tabla, Dato,  DiccionarioString, DiccionarioNat, Lista (cátedra), Conjunto Lineal (cátedra) \\
  ...
  \InterfazFuncion{InsertarEntrada}{\In{r}{registro}, \In{s}{string}, \Inout{b}{base}}{}
  ...
  [\\
    \tab Si $c$ está indexado en alguna de las tablas y es tipo string $\implies$ $O\big((n+m) * |L|)\big)$\\
    \tab Si $c$ está indexado y es nat $\implies$ $O\big((n+m) * (|L|\ +\ log\ (n+m))\big)$\\
    \tab Si no está indexado $\implies$ $O\big(n * m * |L|)\big)$ \\
    \tab Donde L es el dato string más largo en ambas tablas, n y m sus respectivas cantidades de registros.]
  [unión de todos los registros (en caso de que ambas tablas tengan campos con mismo nombre no claves, desempata para $t_1$).]
  [no hay aliasing.]

  \InterfazFuncion{GenerarVistaJoin}{\In{t_1}{string}, \In{t_2}{string}, \In{c}{campo}, \Inout{b}{base}}{itConj(registro)}
  ...
  {$b$ $\igobs$ generarVistaJoin($t_1$, $t_2$, $c$, $b_0$) $\land$ alias$\big($$res$ $\igobs$ CrearIt(vistaJoin($t_1$, $t_2$, $b$))$\big)$}
  [\\
  ...
  [devuelve la unión por copia de dos registros, pero sin campos repetidos.]

  \InterfazFuncion{VistaJoin}{\In{t_1}{string}, \In{t_2}{string}, \In{b}{base}}{itConj(registro)}
  [hayJoin?($t_1$, $t_2$, $b$)]
  {alias($res$ $\igobs$ CrearIt(vistaJoin($t_1$, $t_2$, $b$)))}
  [\\
...
\tab $R$, cantidad de registros a 'actualizar' (unión de las listas de cambios de ambas tablas)]
  ...
  [\\
  \tab  campoIndice nat $\implies$ $O\big(log\ n\ +\ n * |L|\big)$. $O(log\ n\ +\ |L|)$ si además es clave.\\
  \tab campoIndice string $\implies$ $O\big(|L|\ +\ n * |L|\big)$ = $O(n * |L|)$. $O(|L|)$ si además es clave.\\
  \tab Sin campoIndice $\implies$ $O(n * |L|)$\\
  \\
  \tab Donde $L$ es la longitud de dato string más largo de $t$ y $n$ su cantidad de registros.]
  [devuelve por copia una lista de todos los registros de $t$ que coinciden en todos los campos con el registro $criterio$.]
  \footnote{Las complejidades exportadas para casos no clave están consideradas en el caso particular de que todos los registros tengan los mismos índices (ver algoritmo)}

  \InterfazFuncion{CoincidenTodosCrit}{\In{crit}{registro}, \In{r}{registro}}{bool}
  [campos($crit$) $\subset$ campos($r$) $\land$ ($\exists$ $t$: Tabla, nombre($t$) $\in$ tablas($b$)) $r$ $\in$ registros($t$)]
...
\noindent\tab 8) El campo del join es clave para los dos. \\
        ...
        \bigpar{
          ...
          \bigpar{
            ...
            \bigpar{
              ...
              estaAgregado?$\Big(\Pi_1$($t$), obtener$\big(s_2$, obtener($s_1$, $e$.hayJoin)$\big)$.cambiosT1$\Big)$ $\ssi$ \\
...
\quad\quad Las operaciones \& y * para el tipo primitivo puntero cuestan $O(1)$. \\
       ...
       \quad\quad En el peor caso se agrega por copia el registro a la lista de cambios de todas las demás tablas (asumiendo que tiene joins con todas) dos veces. Eso equivale a $O(\#campos * dato\ mas\ costoso\ de\ copiar)$ por cada inserción, pero como los registros tienen cantidad de campos acotados, se reduce la complejidad a $O(L)$. Por lo tanto el ciclo cuesta $O\big(L * \#tablas(b)\big)$.}
     \end{algorithmic}
...
\begin{algorithm}[H]{\textbf{iBorrar}(\In{cr}{registro}, \In{t}{string}, \Inout{e}{estr})}
  \begin{algorithmic}
    ...
    \State $iter \gets VistaDicc\big(Obtener(t, e.hayJoin)\big)$   \Comment $O(1)$
    \While{$HaySiguiente?(iter)$}          \Comment $O(\#tablas * ...)$
      \State $AgregarAtras\big(<cr, false>, (Siguiente(iter).significado).cambiosT1\big)$\\         \Comment $O\big(copy(cr)\big) = O(\#campos * dato\ mas\ costoso) = O(L)$
      \State $Avanzar(iter)$           \Comment $O(1)$
    \EndWhile
    \State $iter \gets CrearIt\big(e.tablas)\big)$   \Comment $O(1)$
    \While{$HaySiguiente?(iter)$}          \Comment $O(\#tablas * ...)$
      \If{$Def?\big(t, Obtener(siguiente(iter), e.hayJoin)\big)$}       \Comment $O(1)$
        \State $cambios \gets Obtener\big(t, Obtener(siguiente(iter), e.hayJoin)\big)$                \Comment $O(1)$
        \State $agregarAtras\big(<cr,false>, (cambios.significado).cambiosT2\big)$                \Comment $O(L)$
      \EndIf
      \State $Avanzar(iter)$           \Comment $O(1)$
    \EndWhile
...
\begin{algorithmic}
   ...
   \quad\quad Si $c$ está indexado en alguna de las tablas y es tipo string $\implies$ $O(n * |L|)$ = $O\big((n+m) * |L|\big)$ \\
   \quad\quad Si $c$ está indexado y es nat $\implies$ $O\big(n * (log\ m\ +\ |L|)\big)$ = $O\big((n+m)*(log(n+m)\ +\ |L|)\big)$ \\
   \quad\quad Si no está indexado $\implies$ $O(n * m * |L|)$ \\
   \\
   ...
   \quad\quad Esta indeterminación se puede salvar considerando que las cantidades de registros de las tablas (que ahora sí serían n y m) pueden ser tomadas como $O\big(max(n,m)\big)$ = $O(n+m)$.\\
   \quad\quad La cantidad de registros mergeados está acotada tanto por $m$ como por $n$ por ser intersección.}
   \Statex \underline{Justificación:} {\\
  \quad\quad Siempre se itera linealmente una tabla, y se realizan búsquedas sobre la otra (complejidad variable exportada por buscar de tabla, según campo indexado o no).}
  \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]{\textbf{iGenerarVistaJoin}(\In{t_1}{string}, \In{t_2}{string}, \In{c}{campo}, \Inout{e}{estr}) $\to$ $res$ : itConj(registro)}
 \begin{algorithmic}
   ...
   \If{$TipoCampo(c, tabla1)$} \Comment $O(1)$
     ...
     \State $regsMergeados \gets CombinarRegistros(t_1, t_2, c)$ \Comment $c\ esta\ indexado\ en\ alguna\ tabla\ \implies\ O\big((n+m) * (log(m+n) +\ |L|)\big)$
   ...
   \Else
     ...
     \State $regsMergeados \gets CombinarRegistros(t_1, t_2, c)$ \Comment $c\ esta\ indexado\ en\ alguna\ tabla\ \implies\ O((n+m) * |L|)$
     \State $ $                                             \Comment $Si\ no\ esta\ indexado\ \implies\ O\big(n * m * |L|\big)$
     \State $it \gets CrearIt(regsMergeados)$                \Comment $O(1)$
     \While{$HaySiguiente(it)$}                            \Comment $O(n * ...)$
   ...
   \medskip
   \Statex \underline{Complejidad:}{\\
\quad\quad Si $c$ está indexado en alguna de las dos tablas y es string $\implies$ $O\big((n+m) * L\ +\ n * L\big)$ = $O((n+m) * L)$ \\
\quad\quad Si $c$ está indexado y es nat $\implies$ $O\big((n+m) * (log\ (n+m) +\ |L|) + n * (log\ n\ +\ L)\big)$ = $O\big((n+m) * (log\ (n+m) + L)\big)$ = $O\big((n+m) * (log(n+m)\ +\ L)\big)$\\
\quad\quad Si $c$ no está indexado $\implies$ $O\big(n * m * |L| + n * (log\ n + L)\big)$ = $O\big(n * (m * L\ +\ log\ n\ +\ L)\big)$ = $O\big(n * (m * L\ +\ log\ n)\big)$ \\
\\
\quad\quad Siendo $L$ el mayor largo de string entre registros, $n$ y $m$ la cantidad de registros de ambas tablas (t1 y t2 respectivamente) \\
}
   \Statex \underline{Justificación:}{\\
...
\quad\quad La cantidad de registros mergeados está acotada tanto por $m$ como por $n$ por ser intersección (en este caso usamos n). \\
\quad\quad Para el caso no indexado tomamos el peor caso entre nat y string para agregar a conjunto y diccionario. Tomamos para eso $O(n*max(log(n), L)) = O(n*(log(n)+ L)) $
}
 \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]{\textbf{iTablas}(\In{e}{estr}) $\to$ $res$ : itConj(string)}
  \begin{algorithmic}
    ...
    \If{$esNat$}
      ...
      \While{$HaySiguiente?(itT1)$} \comentariocompl{R regs. en '.cambiosT1' de s1 y s2}{$O(R * ...)$}
        ...
        \If{$\NOT Vacia?(coincidencias)$} \Comment $O(1)$
          ...
          \State $regT2 \gets Primero(coincidencias)$ \Comment $O(1)$

          \If{$tupSiguiente.agregar?$} \Comment $O(1)$
            \State $registroMergeado \gets Merge(tupSiguiente.reg, regT2)$ \Comment $O(L)$
  ...
  \begin{algorithmic}
      ...
      \While{$HaySiguiente?(itT2)$} \Comment $O(R * ...)$
        ...
        \If{$\NOT Vacia?(coincidencias)\ \AND \NOT Def?(claveNat, diccIters)$} \Comment $O\big(log(n+m)\big)$
          \comentario{CampoJoin siempre es clave, \#coincidencias es 1}
          \State $regT1 \gets Primero(coincidencias)$ \Comment $O(1)$
          \If{$tupSiguiente.agregar?$} \Comment $O(1)$
            \State $registroMergeado \gets Merge(regT1, tupSiguiente.reg)$ \Comment $O(L)$
            \State $iter \gets AgregarRapido\big(registroMergeado, Obtener(s_2, Obtener(s_1, e.registrosDelJoin))\big)$\\ \Comment $O\big(copy(reg)\big) = O(L)$
...
\begin{algorithm}[H]{\textbf{iBusquedaCriterio}(\In{criterio}{Registro}, \In{t}{string}, \In{b}{estr}) $\to$ $res$ : conj(registro)}
 \begin{algorithmic}
    ...
    \State $res \gets Vacio()$                              \Comment $O(1)$
    \comentario{Si los campos de crit no están contenidos en los de tabla -> devuelvo conj vacio}
    \State $itCrit \gets VistaDicc(criterio) $  \Comment $O(1)$
    \While{$HaySiguiente(itCrit)\ \AND \NOT termine $}  \Comment $O(\#campos(t)*..)=O(1)$
      \State $campoCrit \gets Siguiente(itCrit).clave$ \Comment $O(1)$
      \If{$\NOT Pertenece?(campoCrit, Registros(tabla))  $} \Comment $O(\#campos(t))=O(1)$
        \State $termine \gets true $ \Comment $O(1)$
      \EndIf
      \State $Avanzar(itCrit) $ \Comment $O(1)$
    \EndWhile
    \State $ $
    \State $itIndices \gets CrearIt(Indices(tabla))$               \Comment $O(1)$
    \comentario{Por si hay algún campo indexado para facilitar búsqueda}
    \While{$HaySiguiente(itIndices)\ \AND \NOT termine$}    \Comment $O(\#regs * ...) = O(1 * ...)$
        \If{$Def?\big(Siguiente(itIndices), crit\big)$}                   \Comment $O(1)$
            \State $campoIndice \gets Siguiente(itIndices)$            \Comment $O(1)$
            \State $valorCampo \gets Obtener(campoIndice, crit)$    \Comment $O(|$max nombre de campo$|) = O(1)$
            ...
            \State $ $
            \State $itCoincis \gets CrearIt(coincis)$           \Comment $O(1)$
            \While{$HaySiguiente(itCoincis)$}                 \Comment $O(\#coincis * ...)$
                \If{$CoincidenTodosCrit\big(crit, Siguiente(itCoincis)\big)$} \Comment $O(L)$
                    \State $AgregarRapido\big(res, Siguiente(itCoincis)\big)$ \Comment $O(L)$
                \EndIf
                \State $Avanzar(itCoincis)$ \Comment $O(1)$
            \EndWhile
            \State $termine \gets true$  \Comment $O(1)$
            \State $Avanzar(itIndices)$ \Comment $O(1)$
        \EndIf
    \EndWhile
    \State $ $
...
\begin{algorithm}
  \begin{algorithmic}
   ...
   \Statex \underline{Complejidad:} {\\
   \quad\quad campoIndice nat $\implies$ $O\big(log\ n\ +\ |L|\ +\ \#coincis * |L|\big)$ = $O\big(log\ n\ +\ \#coincis * |L|\big)$. $O(log\ n\ +\ |L|)$ si además es clave.\\

    \quad\quad campoIndice string $\implies$ $O\big(|L|\ +\ \#coincis * |L|\big)$ = $O(\#coincis * |L|)$. $O(|L|)$ si además es clave.\\
   ...
   \Statex \underline{Justificación:} {\\
   \quad\quad Como los campos están acotados en nombre (independientemente de si son o no de tabla), obtener un dato en un registro es $O(1)$. \\
   \quad\quad Cuando se chequea en campos de criterio que estén definidos en campos(t), la iteración termina cuando o bien todos están o hay alguno que no. Esto significa que nunca se itera más de de \#campos(t) veces.\\
   \quad\quad Si los campos de criterio están contenidos en los campos de la tabla, implica entonces que los primeros están acotados también en cantidad. \\
   \quad\quad Si campoIndice es clave, entonces \#coincis = 1, de no serlo está acotada por $n$ (la cantidad de registros de la tabla). \\
   \quad\quad Los registros de tabla tienen cantidad acotada de campos, copiar uno por lo tanto es el costo de sus string más largo.\\
   \quad\quad Operaciones Def? y Obtener para todos los registros (con nombres campos acotados por enunciado) es $O(1)$.}
 \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]{\textbf{iCoincidenTodosCrit}(\In{crit}{registro}, \In{r}{registro}) $\to$ $res$ : bool}
 \begin{algorithmic}
   ...
   \While{$HaySiguiente(itCrit) \AND res$}  \Comment $O(\#campos(crit) * ...)$
     ...
     \If{$\NOT Obtener(tuplaCrit.clave, r) = tuplaCrit.significado$}  \Comment $O(L)$
