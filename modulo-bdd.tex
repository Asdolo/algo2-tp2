...
\begin{Interfaz}
  ...
  [\#campos($cr$) = 1 $\land$ $t$ $\in$ tablas($b$) $\land$ $b$ = $b_0$ $\land$ dameUno(campos($cr$)) $\in$ claves(dameTabla(t,b)) ]
...
\tab Campo no indexado $\implies$  $O\big(|L| * (n + \#tablas(b))\big)$, donde $L$ es el dato string más largo de $cr$ y $n$ es la cantidad de registros en la tabla.]
  ...
  {$b$ $\igobs$ generarVistaJoin($t_1$, $t_2$, $c$, $b_0$) $\land$ alias(secuAConj(Siguientes($res$)) $\igobs$ vistaJoin($t_1$, $t_2$, $b$))}
  [\\
\tab $c$ cualquier tipo indexado en $t_1$ y $t_2$ $\implies$ $O\big((n+m) * \big(L + log(n+m)\big)\big)$ \\
...
\tab Donde $n$ = \#registros($t_1$), $m$ = \#registros($t_2$) y $L$ el dato string más largo de cualquiera de las dos tablas.]
  [crea un join entre dos tablas de la base de datos.]
  [res no es modificable, se itera solo a modo de vista del conjunto]
  ...
  [
\newline
\quad\quad Join campo nat indexado $\implies$ $O\big(R*\big(|L| + log(n*m)\big)\big)$\\
\\
\quad\quad Join campo nat no indexado $\implies$ $O\big(R*\big(|L|*(m+n) + log(n+m)\big)\big)$\\
\\
\quad\quad Join campo string indexado $\implies$ $O(R*|L|)$\\
\\
\quad\quad Join campo string no indexado $\implies$ $O\big(R*|L|*(n+m)\big)$
\newline
...
\quad\quad $R$, cantidad de registros a 'actualizar' (unión de las listas de cambios de ambas tablas)\\  ]
  ...
  [$O(1)$]
  [evuelve el nombre de la tabla más accedida.]
...
\begin{Representacion}
  ...
  \noindent\tab 1) Las claves de nombreATabla están en tablas, sus significados tienen su nombre y son todas las tablas de la lista $e$.tablas. \\
\noindent\tab 2) La tabla más accedida está entre las tablas y tiene más accesos que todas las demás. \\
\noindent\tab 3) Las claves de JoinPorCampo, hayJoin y registrosDelJoin son las tablas de $e$.tablas (por (1), lo mismo que claves de $e$.nombreATabla) y las tablas con las que tienen joins a su vez también son de tablas de la base. \\
...
\noindent\tab 6) Los registros del join se crean por combinación de sus tablas \\
\noindent\tab 7) Entre dos tablas solamente puede haber un único join (en una dirección). \\
...
\noindent\tab 12) Los registros en la lista de cambios tienen por campos a los campos de la primer clave y, para cada última aparación de un registro en la lista, el bool agregar refleja si pertenece el registro o no a los registros de las primer clave (para ambas listas). \\
...
\begin{algorithm}[H]{\textbf{iGenerarVistaJoin}(\In{t_1}{string}, \In{t_2}{string}, \In{c}{campo}, \Inout{e}{estr}) $\to$ $res$ : itConj(registro)}
  ...
  \begin{algorithmic}
    \comentario{Creo en el diccionario hayJoin de cada tabla la otra tabla.}
    \State $aux \gets <c, Vacio()>$                             \Comment $O(1)$
    \State $Definir\big(t_2, aux, Obtener(t_1, e.hayJoin)\big)$ \Comment $O(|maximo\ nombre\ de\ tabla|) = O(1)$
    \State $Definir\big(t_1, aux, Obtener(t_2, e.hayJoin)\big)$ \Comment $O(|maximo\ nombre\ de\ tabla|) = O(1)$
    \State $ $

    \State $Definir\big(t_2, Vacio(), Obtener(t_1, e.registrosDelJoin)\big)$ \Comment $O(|maximo\ nombre\ de\ tabla|) = O(1)$
    \State $Definir\big(t_1, Vacio(), Obtener(t_2, e.registrosDelJoin)\big)$ \Comment $O(|maximo\ nombre\ de\ tabla|) = O(1)$
    \State $ $
    
    \comentario{Si es nat el campoJoin...}
    \If{$TipoCampo(c, t1)$} \Comment $O(1)$
      \comentario{Defino en cada diccionario de joinPorCampoNat la otra tabla.}
      \State $Definir\big(t_2, Vacio(), Obtener(t_1, e.joinPorCampoNat)\big)$ \Comment $O(1)$
      \State $Definir\big(t_1, Vacio(), Obtener(t_2, e.joinPorCampoNat)\big)$ \Comment $O(1)$
      \State $ $

      \comentario{Itero sobre los registros de t1 buscando en t2 por el registro (es único por ser clave el campo, req) que comparta ese dato (si lo hay), los mergeamos y definimos en el diccionario de t1 a t2 (y viceversa)}
      \State $ $

      \State $it \gets CrearIt(Registros(t_1))$ \Comment $O(1)$
      \While{$HaySiguiente(it)$} \Comment $O(n * ...)$
        \State $d \gets Obtener\big(c, Siguiente(it)\big)$ \Comment $O(1)$
        \State $coincis \gets Buscar(c, d ,t_2)$ \Comment $Si\ esta\ indexado\ \implies O(log\ m + |L|)\ /\ Si\ no\ esta\ indexado\ \implies O(m * |L|)$
        \If{$\NOT Vacia?(coincis)$} \Comment $O(L)$
          \State $regMergeado \gets Merge\big(Siguiente(it), Primero(coincis)\big)$ \Comment $O(L)$
          \comentario{Agrego a los dos conjuntos de registros (son iguales)}
          \State $iter_1 \gets AgregarRapido\big(regMergeado, Obtener\big(t_2, Obtener(t_1, e.registrosDelJoin)\big)\big)$ \Comment $O(L)$
          \State $iter_2 \gets AgregarRapido\big(regMergeado, Obtener\big(t_1, Obtener(t_2, e.registrosDelJoin)\big)\big)$ \Comment $O(L)$
          \State $n \gets ValorNat(d)$ \Comment $O(1)$
          \State $Definir\big(n, iter_1, Obtener\big(t_2, Obtener(t_1, e.joinPorCampoNat)\big)\big)$ \Comment $O(log\ n)$
          \State $Definir\big(n, iter_2, Obtener\big(t_1, Obtener(t_2, e.joinPorCampoNat)\big)\big)$ \Comment $O(log\ n)$
        \EndIf
        \State $Avanzar(it)$ \Comment $O(1)$
      \EndWhile
      \State $ $

      \comentario{Itero sobre registros de t2 igual que arriba pero además de preguntando si ya los definimos antes}
      \State $ $

      \State $it \gets CrearIt(Registros(t_2))$ \Comment $O(1)$
      \While{$HaySiguiente(it)$} \Comment $O(m * ...)$
        \State $d \gets Obtener\big(c, Siguiente(it)\big)$ \Comment $O(1)$
        \State $coincis \gets Buscar(c, d ,t_1)$ \Comment $Si\ esta\ indexado\ \implies O(log\ n + |L|)\ /\ Si\ no\ esta\ indexado\ \implies O(n * |L|)$
        \State $n \gets ValorNat(d)$
        \If{$\NOT Vacia?(coincis)\ \AND \NOT Def?\big(n, Obtener\big(t_1, Obtener(t_2, e.joinPorCampoNat)\big)\big)$}\\ \Comment $O(log(n+m))$
          \State $regMergeado \gets Merge\big(Siguiente(it), Primero(coincis)\big)$ \Comment $O(L)$
          \comentario{Agrego a los dos conjuntos de registros (son iguales)}
          \State $iter_1 \gets AgregarRapido\big(regMergeado, Obtener\big(t_2, Obtener(t_1, e.registrosDelJoin)\big)\big)$ \Comment $O(L)$
          \State $iter_2 \gets AgregarRapido\big(regMergeado, Obtener\big(t_1, Obtener(t_2, e.registrosDelJoin)\big)\big)$ \Comment $O(L)$
          \State $Definir\big(n, iter_1, Obtener\big(t_2, Obtener(t_1, e.joinPorCampoNat)\big)\big)$ \Comment $O(log\ n+m)$
          \State $Definir\big(n, iter_2, Obtener\big(t_1, Obtener(t_2, e.joinPorCampoNat)\big)\big)$ \Comment $O(log\ n+m)$
        \EndIf
        \State $Avanzar(it)$ \Comment $O(1)$
      \EndWhile
      \State $ $
    ...
    \Else
      ...
      \comentario{mismo que arriba pero para joinPorCampoString}
      \State $Definir\big(t_2, Vacio(), Obtener(t_1, e.joinPorCampoString)\big)$ \Comment $O(1)$
      \State $Definir\big(t_1, Vacio(), Obtener(t_2, e.joinPorCampoString)\big)$ \Comment $O(1)$
      \State $ $
...
\begin{algorithm}
  \begin{algorithmic}
      ...
      \comentario{Itero sobre registros de t1 buscando en t2 por cada clave el registro (es único por ser clave el campo, req) que comparta ese dato (si lo hay) y los mergeamos y definimos en el diccionario de t1 a t2 (y viceversa)}
      \State $ $

      \State $it \gets CrearIt(Registros(t_1))$ \Comment $O(1)$
      \While{$HaySiguiente(it)$} \Comment $O(n * ...)$
        \State $d \gets Obtener\big(c, Siguiente(it)\big)$ \Comment $O(1)$
        \State $coincis \gets Buscar(c, d ,t_2)$ \Comment $Si\ esta\ indexado\ \implies O(|L|)\ /\ Si\ no\ esta\ indexado\ \implies O(m * |L|)$
        \If{$\NOT Vacia?(coincis)$} \Comment $O(1)$
          \State $regMergeado \gets Merge\big(Siguiente(it), Primero(coincis)\big)$ \Comment $O(L)$
          \State $s \gets ValorStriing(d)$ \Comment $O(L)$
          \comentario{Agrego a los dos conjuntos de registros (son iguales)}
          \State $iter_1 \gets AgregarRapido\big(regMergeado, Obtener\big(t_2, Obtener(t_1, e.registrosDelJoin)\big)\big)$ \Comment $O(L)$
          \State $iter_2 \gets AgregarRapido\big(regMergeado, Obtener\big(t_1, Obtener(t_2, e.registrosDelJoin)\big)\big)$ \Comment $O(L)$
          \State $Definir\big(s, iter_1, Obtener\big(t_2, Obtener(t_1, e.joinPorCampoString)\big)\big)$ \Comment $O(L)$
          \State $Definir\big(s, iter_2, Obtener\big(t_1, Obtener(t_2, e.joinPorCampoString)\big)\big)$ \Comment $O(L)$
        \EndIf
        \State $Avanzar(it)$ \Comment $O(1)$
      \EndWhile
      \State $ $
      ...
      \While{$HaySiguiente(it)$} \Comment $O(m * ...)$
        \State $d \gets Obtener\big(c, Siguiente(it)\big)$ \Comment $O(1)$
        \State $coincis \gets Buscar(c, d ,t_1)$ \Comment $Si\ esta\ indexado\ \implies O(|L|)\ /\ Si\ no\ esta\ indexado\ \implies O(n * |L|)$
        \State $s \gets ValorString(d)$
        \If{$\NOT Vacia?(coincis)\ \AND \NOT Def?\big(s, Obtener\big(t_1, Obtener(t_2, e.joinPorCampoString)\big)\big)$}\\ \Comment $O(L)$
          \State $regMergeado \gets Merge\big(Siguiente(it), Primero(coincis)\big)$ \Comment $O(L)$
          \comentario{Agrego a los dos conjuntos de registros (son iguales)}
          \State $iter_1 \gets AgregarRapido\big(regMergeado, Obtener\big(t_2, Obtener(t_1, e.registrosDelJoin)\big)\big)$ \Comment $O(L)$
          \State $iter_2 \gets AgregarRapido\big(regMergeado, Obtener\big(t_1, Obtener(t_2, e.registrosDelJoin)\big)\big)$ \Comment $O(L)$
    ...
    \EndIf
    \State $res \gets CrearIt\big(Obtener\big(t_2, Obtener(t_1, e.registrosDelJoin)\big)\big)$

    \medskip
    \Statex \underline{Complejidad:}{ \\
\quad $c$ tipo nat indexado en $t_1$ y $t_2$ $\implies$ \\ 
\quad $O\big(n*( L+ log n + log m)\big)$ + $O\big(m*(L + log n + log(n+m)\big)$ \\
\quad = $O\big(n*(L + log n + log m)\big)$ + $O\big(m*\big(L + log(n+m)\big)\big)$ \\
\quad = $O\big(n*\big(L + log (n+m) + log (n+m)\big)\big)$ + $O\big(m*\big(L + log(n+m)\big)\big)$ \\
\quad = $O\big(n*\big(L + log(n+m)\big)\big)$ + $O\big(m*\big(L +log(n+m)\big)\big)$ \\
\quad = $O\big((n+m) * \big(L + log(n+m)\big)\big)$\\
\\
\quad $c$ tipo nat no indexado $\implies$ \\
\quad $O\big(n*(L + log n + m*L)\big)$ + $O\big(m*(L + n*L + log (n+m))\big)$ \\
\quad = $O\big(L(n+m) + n(log n + m*L) + m\big(log (n+m) + n*L\big)\big)$\\
\quad = $O\big(L(n+m) + n\big(log (n+m)+ m*L\big) + m\big(log (n+m) + n*L\big) \big)$ \\
\quad = $O\big(\big(L + log (n+m)\big)(n+m) + n*m*L + n*m*L\big)$ \\
\quad = $O\big((n+m)\big(L + log(n+m)\big) + n*m*L\big)$ \\
\\
\quad $c$ tipo string indexado en $t_1$ y $t_2$ $\implies$ $O(n*L + m*L)$ =  $O\big(L*(n+m)\big)$ \\
\quad $c$ tipo string no indexado $\implies$ $O\big(n*(m*L+L)\big)$ + $O\big(m*(n*L+L)\big)$ = $O\big(L*(m + n + n*m + n*m)\big)$ = $O\big(L*(n + m + m*n)\big)$ = $O(L*m*n)$}
...
\begin{algorithm}
  \begin{algorithmic}
      \algrestore{myalg}
\Statex
{\quad $\implies$ $c$ cualquier tipo indexado en $t_1$ y $t_2$ $\implies$ \\
\quad $O\big(max\{(n+m) * (L + log(n+m)), L*(n+m)\}\big)$ = $O\big((n+m) * \big(L + log(n+m)\big)\big)$ \\
\\
\quad $\implies$ $c$ cualquier tipo no indexado $\implies$ \\
\quad $O\big(max\{(n+m)(L + log(n+m)) + L*n*m, L*m*n\}\big)$ = $O\big((n+m)\big(L + log(n+m)\big) + L*n*m\big)$}
    \Statex \underline{Justificación:}
    Definir el campo y los dos diccionarios vacíos para cada estructura es $O(1)$. \\
\quad Obtener un significado en los diccString que tienen nombres de campos por claves también es $O(1)$ porque están acotados en longitud. \\
\\
\\
\quad \underline{Caso nat:} Se recorren los $n$ registros de $t_1$ buscando coincidencias en $t_2$ para el campo $c$ (si $c$ es indexado en $t_2$ entonces esto se realiza en tiempo promedio logarítmico sobre la cantidad $m$ de registros de $t_2$, si no es lineal con el agregado de copiar coincidencias considerando peor caso recorrer todos los registros). \\
\quad Si hay coincidencias se deben mergear ambos registros en $O(L)$, siendo $L$ el dato string más largo de cualquiera de las dos tablas, y luego definir el merge en las dos estructuras que almacenan registros de joins en común con complejidad Copy(merge) = $O(L)$ ya que al tener campos acotados sólo es significativo el costo de copiar una cantidad acotada de veces el dato más largo. \\
\quad Se define para cada registro mergeado un iterador en cada diccNat de la estructura de joinPorCampoNat (como solamente se agregaron iteradores de registros de $t_1$ hasta ahora, log\ n peor caso). \\
\quad Luego se aplica el mismo procedimiento para los $m$ registros de $t_2$, pero esta vez considerando que no hayan sido agregados en el paso anterior, por lo que se agrega una búsqueda sobre la estructura de registros de joins $\big($peor caso contiene todos los registros de ambas tablas: $O(log (n+m)\big)$. \\
\\
\quad \underline{Caso string}: Similar al caso nat pero considerando que las búsquedas e inserciones se realizan en tiempo de orden $L$ (nuevamente, el dato string más largo de cualquiera de las dos tablas).
  \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]{\textbf{iBorrarJoin}(\In{t_1}{string}, \In{t_2}{string}, \Inout{e}{estr})}
  \begin{algorithmic}
    ...
    \State $Borrar\big(t_1, Obtener(t_2, e.hayJoin)\big)$             \Comment $O(1)$
    \State $Borrar\big(t_2, Obtener(t_1, e.registrosDelJoin)\big)$    \Comment $O(1)$
    \State $Borrar\big(t_1, Obtener(t_2, e.registrosDelJoin)\big)$    \Comment $O(1)$
    \If{$Def?\big(t_2, Obtener(t_1, e.joinPorCampoNat)\big)$}         \Comment $O(1)$
      ...
      \State $Borrar\big(t_1, Obtener(t_2, e.joinPorCampoNat)\big)$   \Comment $O(1)$
    \Else
      \State $Borrar\big(t_2, Obtener(t_1, joinPorCampoString)\big)$  \Comment $O(1)$
      \State $Borrar\big(t_1, Obtener(t_2, joinPorCampoString)\big)$  \Comment $O(1)$
    \EndIf
...
\begin{algorithm}[H]{\textbf{iVistaJoin}(\In{s_1}{string}, \In{s_2}{string}, \In{b}{estr}) $\to$ $res$ : itConj(registro)}
  \begin{algorithmic}
    ...
    \State $esNat \gets TipoCampo?\Big(Obtener\big(s_2, \big(Obtener(s_1.hayJoin)\big)\big), Obtener(s_1, b.nombreATabla)\Big)$             \Comment $O(1)$
    \State $ $

    \comentario{campito = CAMPOJOIN}
    \State $campito \gets Obtener\big(s_2, \big(Obtener(s_1.hayJoin)\big)\big).campoJoin$
    \State $ $

    \comentario{Creamos un iterador a la lista de cambios de tipo <registro, bool> de s1}
    \State $iteChanges \gets CrearIt\Big(obtener\big(s_2,\big(obtener(s_1,b.hayJoin)\big)\big).cambios\Big)$ \Comment $O(1)$
    \State $ $

    \If{$esNat$}
      \comentario{Join por campo nat}
      \State $diccDeIters1 \gets Obtener\big(s_1, Obtener\big(s_2, b.joinPorCampoNat\big)\big)$ \Comment $O(1)$
      \State $diccDeIters2 \gets Obtener\big(s_2, Obtener\big(s_1, b.joinPorCampoNat\big)\big)$ \Comment $O(1)$
      \State $ $

      \comentario{Guardo o elimino los registros en el join}
      \comentario{Si no hay ninguno $\implies$ No actualizo nada $\implies$ O(1)}

      \While{$HaySiguiente?(iteChanges)$} \comentariocompl{R regs. en '.cambios' de s1 y s2}{$O(R * ...)$}
        \State $tupSiguiente \gets Siguiente(itChanges)$ \Comment $O(1)$
        ...
        \State $coincidencias \gets Buscar(campito, claveNat, nombreATabla(s_2))$\\ \Comment $Campo\ indexado \implies O(log\ m + |L|)\ /\ Campo\ no\ indexado \implies O(m * |L|)$
        \If{$\#coincidencias > 0$} \Comment $O(1)$
          \comentario{como campoJoin siempre es clave, \#coincidencias es 1}
          \comentario{regTablaActual es el registro en s2 que coincide en 'campito' con claveNat}
          ...
          \If{$tupSiguiente.agregar?$} \Comment $O(1)$
            ...
            \State $iter1 \gets AgregarRapido\big(registroMergeado, Obtener(s_2, Obtener(s_1, e.registrosDelJoin))\big)$\\ \Comment $O\big(copy(reg)\big) = O(L)$
            \State $iter2 \gets AgregarRapido\big(registroMergeado, Obtener(s_1, Obtener(s_2, e.registrosDelJoin))\big)$ \Comment $O(L)$

            \State $Definir(claveNat, iter1, diccDeIters1)$\\ \comentariocompl{m regs en s2, n regs en s1}{$O\big(log\ (n+m)\big)\ +\ O\big(copy(iter)\big) = O\big(log(n+m)\big)$}
            \State $Definir(claveNat, iter2, diccDeIters2)$ \Comment $O\big(log(n+m)\big)$
          \Else
            \State $EliminarSiguiente\big(Obtener(claveNat, diccDeIters1)\big)$ \Comment $O\big(log(n+m)\big)$
            \State $EliminarSiguiente\big(Obtener(claveNat, diccDeIters2)\big)$ \Comment $O\big(log(n+m)\big)$
            \State $Borrar(claveNat, diccDeIters1)$ \Comment $O\big(log(n+m)\big)$
            \State $Borrar(claveNat, diccDeIters2)$ \Comment $O\big(log(n+m)\big)$
          \EndIf
        \EndIf
        \State $EliminarSiguiente(iteChanges)$ \Comment $O(1)$
      \EndWhile
      \State $ $

      \comentario{Ahora hago al reves, me fijo de la tabla 2 a la tabla 1}
      \State $iteChanges \gets CrearIt\Big(obtener\big(s_1,\big(obtener(s_2,b.hayJoin)\big)\big).cambios\Big)$ \Comment $O(1)$
      \While{$HaySiguiente?(iteChanges)$} \comentariocompl{R regs. en '.cambios' de s1 y s2}{$O(R * ...)$}
        \State $tupSiguiente \gets Siguiente(itChanges)$ \Comment $O(1)$
        \State $claveNat \gets Obtener(campito, tupSiguiente.reg)$ \Comment $O(1)$
        \State $ $
...
\begin{algorithm}
  \begin{algorithmic}
      \algrestore{myalg}
        ...
        \comentario{Pregunto si esta definido para no agregar el registro dos veces, solo pregunto en un dicc porque si esta definido en uno esta definido en el otro}
        \State $coincidencias \gets Buscar(campito, claveNat, nombreATabla(s_1))$\\ \Comment $Campo\ indexado \implies O(log\ n + |L|)\ promedio\ /\ Campo\ no\ indexado \implies O(n * |L|)$
        \If{$\#coincidencias > 0\ \AND\ \NOT Def?\big(claveNat, diccIters1\big)$} \Comment $O\big(log(n+m)\big)$
          \comentario{Como campoJoin siempre es clave, \#coincidencias es 1}
          ...
          \If{$tupSiguiente.agregar?$} \Comment $O(1)$
            ...
            \State $Definir(claveNat, iter1, diccDeIters1)$\\ \Comment $O\big(log(n+m)\big)$
            \State $Definir(claveNat, iter2, diccDeIters2)$ \Comment $O\big(log(n+m)\big)$
          \Else
            \State $EliminarSiguiente\big(Obtener(claveNat, diccDeIters1)\big)$ \Comment $O\big(log(n+m)\big)$
            \State $EliminarSiguiente\big(Obtener(claveNat, diccDeIters2)\big)$ \Comment $O\big(log(n+m)\big)$
        ...
        \State $EliminarSiguiente(iteChanges)$ \Comment $O(1)$
      \EndWhile
      \State $res \gets CrearIt\big(Obtener\big(s_1, Obtener(s_2, e.registrosDelJoin)\big)\big)$ \Comment $O(1)$
    ...
    \Else
      ...
      \State $diccDeIters1 \gets Obtener\big(s_1, Obtener\big(s_2, b.joinPorCampoString\big)\big)$ \Comment $O(1)$
      \State $diccDeIters2 \gets Obtener\big(s_2, Obtener\big(s_1, b.joinPorCampoString\big)\big)$ \Comment $O(1)$
      \State $ $

      \While{$HaySiguiente?(iteChanges)$} \Comment $O(R * ...)$
        \State $tupSiguiente \gets Siguiente(itChanges)$ \Comment $O(1)$
        \State $claveString \gets Obtener(campito, tupSiguiente.reg)$ \Comment $O(1)$
        \State $coincidencias \gets Buscar(campito, claveString, nombreATabla(s_2))$\\ \Comment $Campo\ indexado \implies O(L|)\ /\ Campo\ no\ indexado \implies O(m * |L|)$
        \If{$\#coincidencias > 0$} \Comment $O(1)$
          \comentario{como campoJoin siempre es clave, \#coincidencias es 1}
          \State $regTablaActual \gets Primero(coincidencias)$ \Comment $O(1)$
          \If{$tupSiguiente.agregar?$} \Comment $O(1)$
            ...
            \State $Definir(claveString, iter1, diccDeIters1)$\\ \Comment $O(L)\ +\ O\big(copy(iter)\big) = O(L)$
            \State $Definir(claveString, iter2, diccDeIters2)$ \Comment $O(L)$
          \Else
            \State $EliminarSiguiente\big(Obtener(claveString, diccDeIters1)\big)$ \Comment $O(L)$
            \State $EliminarSiguiente\big(Obtener(claveString, diccDeIters2)\big)$ \Comment $O(L)$
            \State $Borrar(claveString, diccDeIters1)$ \Comment $O(L)$
            \State $Borrar(claveString, diccDeIters2)$ \Comment $O(L)$
          \EndIf
        \EndIf
        \State $EliminarSiguiente(iteChanges)$ \Comment $O(1)$
      \EndWhile
...
\begin{algorithm}
  \begin{algorithmic}
      ...
      \comentario{Ahora hago al reves, me fijo de la tabla 2 a la tabla 1}
      \State $listCambios \gets obtener\big(s_1,\big(obtener(s_2,b.hayJoin)\big)\big).cambios$ \Comment $O(1)$
      \State $iteChanges \gets CrearIt(listCambios)$ \Comment $O(1)$
      \While{$HaySiguiente?(iteChanges)$} \Comment $O(R * ...)$
        \State $tupSiguiente \gets Siguiente(itChanges)$ \Comment $O(1)$
        \State $claveString \gets Obtener(campito, tupSiguiente.reg)$ \Comment $O(1)$
        \State $coincidencias \gets Buscar(campito, claveString, nombreATabla(s_1))$\\ \Comment $Campo\ indexado \implies O(|L|)\ /\ Campo\ no\ indexado \implies O(n * |L|)$
        \If{$\#coincidencias > 0\ \AND\ \NOT Def?\big(claveString, diccIters1\big)$} \Comment $O\big(|L|)$
          \comentario{Como campoJoin siempre es clave, \#coincidencias es 1}
...
\begin{algorithm}[H]{\textbf{iBusquedaCriterio}(\In{criterio}{Registro}, \In{t}{string}, \In{b}{estr}) $\to$ $res$ : conj(registro)}
  \begin{algorithmic}
    ...
    \Statex \underline{Justificación:} {Su campoIndice es clave entonces $#coincis = 1$, de no serlo está acotada por $n$. \\
    Los registros tienen cantidad acotada de campos, copiar uno por lo tanto es el costo de sus string más largo.
