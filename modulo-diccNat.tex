...
\medskip
  ...
  \Rep[iter][i]{$\big(\forall$ $n$) : estrDiccNat, estaEnPila?($n$, $i$)$\big)$ $\impluego$ Rep($n$) $\yluego$ \\
  $\Big($noApilaNulos($i$) $\land$ ordenadaDec($i$) $\land$ $\big(\forall$ $n$ : estrDiccNat)(n $\neq$ NULL $\land$ estaEnPila?($n$, $i$) $\impluego $$\big)$ $\neg\big(\exists$ $n'$ : estrDiccNat) ($n'$$\neq$ NULL $\land$ $n$ $\nigobs$ $n'$ $\land$ estaEnPila?($n'$, $i$) $\land $ $\big((\exists$ $x$ : nat) esta?($x$, clavesDe($n$)) $\land$ esta?($x$, clavesDe($n'$))$\big)\Big)$}
...
\begin{algorithm}[H]{\textbf{iSiguientes}(\In{it}{iter}) $\to$ $res$ : lista\big(tupla\big\langle nat, significado\big\rangle\big)}
  \begin{algorithmic}
    \comentario{DFS}
    \State $res \gets Vacia()$ \Comment $O(1)$
    \State $iterador \gets Copiar(it)$ $O(copy(it))$ $\implies$ $O(1)$
