...
\begin{Interfaz}
  ...
  \textbf{servicios usados}\hangindent=2\parindent : Conjunto Lineal (cátedra) \\
...
\begin{algorithm}[H]{\textbf{iMin}(\In{cs}{conj(estrDato)}) $\to$ $res$ : estrDato}
  \begin{algorithmic}
    ...
    \Statex \underline{Justificación:} {Se recorre toda la lista $\big(O(\#cs)\big)$ y cada elemento se compara con el auxiliar $res$. A lo sumo, $res$ se va a comparar con el string de longitud $L$. }
      \end{algorithmic}
\end{algorithm}
