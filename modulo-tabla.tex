...
\begin{Representacion}
  ...
  \noindent\tab 6) Todos los valores de los registros son menores o iguales al maximo y mayor o iguales al minimo para cada campo indexado. \\
  ...
  \Rep[estrTabla][e]{}
    ...
    \noindent\tab\tab $\neg$ ($\exists $ $c $: campo, $c $ $\in $ $e$.claves) \ obtener($c$,$r_1$) $\igobs$ obtener($c$,$r_2$)
    \\
      ...
      \algrestore{myalg}
        ...
        \While{$HaySiguiente(iter)$}                              \Comment $O(n * ...)$
          ...
          \comentario [Borro de los índices, si hay]
            \If{$ \neg Vacia(e.campoIndexadoNat)$}    \Comment $O(1)$
              \State $ regi \gets Siguiente(iter) $ \Comment $O(1)$
              \State $ valorIndex \gets valorNat(Obtener(Primero(e.campoIndexadoNat)).nombre, regi) $ \Comment $O(1)$
              ...
              \While{$ HaySiguiente(itDeIters) $}                                                   \Comment $O(n*..)$
                ...
                \If{$ Siguiente(Siguiente(itDeIters)) = regi $}                                     \Comment $O(1)$
                  \State $ EliminarSiguiente(itDeIters) $                                           \Comment $O(1)$
                \EndIf
              \EndWhile

            \Elsif{$ \neg Vacia(e.campoIndexadoString) $}                                  \Comment $O(1)$
              \State $ regi \gets Siguiente(iter) $                       \Comment $O(1)$
              \State $ valorIndex \gets valorStr(Obtener(Primero(e.campoIndexadoString)).nombre, regi) $  \Comment $O(L)$
...
\begin{algorithm}[H]{\textbf{iNuevaTabla}(\In{nombre}{string}, \In{claves}{conj(campo)}, \In{columnas}{registro}) $\to$ $res$ : estrTabla}
  \begin{algorithmic}
    ...
    \While{$ HaySiguiente(iter) $}                      \Comment $O(\#campos(columnas)*..)=O(1*..)$
      \State $ Definir(Siguiente(iter).clave, Nat?(Siguiente(iter).significado, res.campos)) $
      \State $ $                                        \Comment $O(|$max nombre de campo$|)=O(1)$
      \State $ Avanzar(iter) $                          \Comment $O(1)$
    \EndWhile
