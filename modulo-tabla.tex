...
\begin{Interfaz}
  ...
  \textbf{servicios exportados}: Todos los de la interfaz.

  \textbf{servicios usados}\hangindent=2\parindent : Dato, DiccionarioString, DiccionarioNat, Lista (cátedra), Conjunto Lineal (cátedra) \\
  ...
  \footnote{Es una operación auxiliar tanto para tabla como para BDD, pero no corresponde al buscar de especificación (ver BusquedaCriterio de BDD)}
  \InterfazFuncion{Buscar}{\In{c}{campo}, \In{d}{dato}, \In{t}{tabla}}{Lista(registro)}
  ...
  [Busca en todos los registros de la tabla los que tengan el dato $d$ en el campo $c$, esos registros los devuelve en una lista.]
...
\begin{Representacion}
  ...
  \Rep[estrTabla][e]{}
    ...
    \noindent\tab\tab\bigpar{$\big(\forall$ $c$ : string) ($c$ $\in$ claves($e$.indicesString)$\big)$ $\impluego$
    $\big(\forall$ $r$ : itConj(registro)) ($r$ $\in$ obtener($c$, $e$.indicesString)$\big)$ $\impluego$ \\
